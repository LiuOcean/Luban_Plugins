...
internal static class Program
{
    private class CommandOptions
    {
        [Option(
            ...
            "schemaCollector",
            Required = false,
            HelpText = "schema collector name"
        )]
        ...
        [Option(
            ...
            "forceLoadTableDatas",
            Required = false,
            HelpText = "force load table datas when not any dataTarget"
        )]
        ...
        [Option(
            ...
            "xargs",
            Required = false,
            HelpText = "args like -x a=1 -x b=2"
        )]
    ...
    private static void PrintCopyRight()
    {
        ...
        s_logger.Info("");
        s_logger.Info(
            "    Luban is developed by Code Philosophy Technology Co., LTD. website: https://code-philosophy.com"
        );
        s_logger.Info("");
        s_logger.Info(" =============================================================================================");
    }
    ...
    private static void InitSimpleNLogConfigure(NLog.LogLevel minConsoleLogLevel)
    {
        ...
        if(minConsoleLogLevel <= NLog.LogLevel.Debug)
        {
            layout = NLog.Layouts.Layout.FromString(
                "${longdate}|${level:uppercase=true}|${callsite}:${callsite-linenumber}|${message}${onexception:${newline}${exception:format=tostring}${exception:format=StackTrace}}"
            );
        }
        else
        {
            layout = NLog.Layouts.Layout.FromString(
                "${longdate}|${message}${onexception:${newline}${exception:format=tostring}${exception:format=StackTrace}}"
            );
        }
